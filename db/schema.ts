...
export const issues = pgTable(
  'issues',
  {
    ...
    issueType: varchar('issue_type', { length: 16 }).notNull().default('bug'),
    title: varchar('title', { length: 255 }).notNull(),
    description: text('description'),
    status: varchar('status', { length: 24 }).notNull().default('todo'),
    priority: varchar('priority', { length: 16 }).notNull().default('medium'),
    assigneeGroup: varchar('assignee_group', { length: 24 }),
    assigneeId: uuid('assignee_id').references(() => user.id, { onDelete: 'set null' }),
    testerAssigneeGroup: varchar('tester_assignee_group', { length: 24 }),
    testerAssigneeId: uuid('tester_assignee_id').references(() => user.id, { onDelete: 'set null' }),
    ...
    reopenedById: uuid('reopened_by').references(() => user.id, { onDelete: 'set null' }),
  ...
  (table) => [
    ...
    index('issues_assignee_group_idx').on(table.assigneeGroup),
    index('issues_tester_assignee_group_idx').on(table.testerAssigneeGroup),
    ...
    index('issues_tester_assignee_id_idx').on(table.testerAssigneeId),
    index('issues_reporter_id_idx').on(table.reporterId),
    index('issues_tested_by_id_idx').on(table.testedById),
    index('issues_reopened_by_idx').on(table.reopenedById),
