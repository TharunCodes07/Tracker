...
function normalizeReleaseStatus(value: string | null | undefined): ProjectReleaseStatus {
...
function appendNullableIdFilter(
  ...
  const selectedIds = selectedValues.filter((value) => value !== GENERAL_MODULE_FILTER_VALUE);
...
function buildProjectIssuesWhereClause(
  ...
  appendNullableIdFilter(conditions, issues.componentId, input.componentFilters);
...
function buildProjectIssuesOrderBy(
  ...
  switch (input.sortBy) {
    ...
    case "title":
      return [withDirection(issues.title), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "issueType":
    case "type":
      return [withDirection(typeOrder), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "status":
      return [withDirection(statusOrder), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "priority":
      return [withDirection(priorityOrder), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "module":
    case "moduleName":
      return [withDirection(sql<string>`coalesce(${aliases.moduleName}, '')`), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "component":
    case "componentName":
      return [withDirection(sql<string>`coalesce(${aliases.componentName}, '')`), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "epic":
    case "epicTitle":
      return [withDirection(sql<string>`coalesce(${aliases.epicTitle}, '')`), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "release":
    case "releaseName":
      return [withDirection(sql<string>`coalesce(${aliases.releaseName}, '')`), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "sprint":
    case "sprintName":
      return [withDirection(sql<string>`coalesce(${aliases.sprintName}, '')`), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "assignee":
    case "assigneeName":
    case "assignedToName":
    ...
    case "fixedDate":
      return [withDirection(issues.fixedDate), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "developmentStatus":
      return [withDirection(issues.developmentStatus), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "deploymentStatus":
      return [withDirection(issues.deploymentStatus), desc(issues.updatedAt), desc(issues.sequence), asc(issues.id)];
    case "updatedAt":
    default:
      return direction === "asc"
...
async function getIssueGroupCounts(
  projectId: string,
  field: typeof issues.moduleId | typeof issues.componentId | typeof issues.releaseId | typeof issues.epicId | typeof issues.sprintId
...
async function getIssueSubtaskCounts(issueIds: string[]) {
  ...
  return new Map(
    rows
      ...
      .map((row) => [row.parentIssueId as string, Number(row.subtaskCount ?? 0)])
  );
}
...
async function getIssueCommentCounts(issueIds: string[]) {
  ...
  return new Map(rows.map((row) => [row.issueId, Number(row.commentCount ?? 0)]));
}
...
async function hydrateIssueRows(
  ...
  const [mediaByIssueId, subtaskCounts, commentCounts, commentTexts] = await Promise.all([
...
export async function getProjectIssuesWorkspaceForUser(
  ...
  const [moduleRows, componentRows, releaseRows, sprintRows, epics] = await Promise.all([
    db
      .select({
      ...
      .orderBy(asc(projectModules.sortOrder), asc(projectModules.name), asc(projectModules.id)),
    db
      .select({
      ...
      .innerJoin(projectModules, eq(projectComponents.moduleId, projectModules.id))
...
export async function listProjectIssuesForUser(
  ...
  const [summary, moduleCounts, componentCounts, releaseCounts, epicCounts, sprintCounts, totalItems] =
    await Promise.all([
  ...
  const rows = await getProjectIssueRows(projectId, userId, {
    ...input,
    page,
  });
...
export async function getProjectIssueForUser(
  ...
  const matchedRows = rows.filter((row) =>
    isUuid(issueKeyOrId) ? row.id === issueKeyOrId || row.key === issueKeyOrId : row.key === issueKeyOrId
  );
...
function toIssueExcelRow(issue: IssueListItem): IssueExcelRow {
  return {
    ...
    title: issue.title,
    priority: ISSUE_PRIORITY_OPTIONS.find((option) => option.value === issue.priority)?.label ?? issue.priority,
    assignedToName: issue.assigneeName,
    status: ISSUE_STATUS_OPTIONS.find((option) => option.value === issue.status)?.label ?? issue.status,
    comments: issue.comments,
    remark: issue.remark,
    testedByName: issue.testedByName,
    fixedDate: issue.fixedDate,
    developmentStatus:
      DEVELOPMENT_STATUS_OPTIONS.find((option) => option.value === issue.developmentStatus)?.label ??
      issue.developmentStatus,
    deploymentStatus:
      DEPLOYMENT_STATUS_OPTIONS.find((option) => option.value === issue.deploymentStatus)?.label ??
      issue.deploymentStatus,
...
export async function listProjectIssueWorkbookBundleForUser(
  ...
  const rows = await listProjectIssuesForExcelForUser(userId, teamId, projectId, {
    page: 1,
    pageSize: 2147483647,
    search: "",
    resolution: "all",
