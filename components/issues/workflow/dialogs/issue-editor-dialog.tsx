...
import { joinBulletItems, splitBulletItems } from "@/components/issues/shared/issue-text";
...
const workflowSections = [
  { id: "issue", label: "Issue", icon: ClipboardList },
  { id: "planning", label: "Planning", icon: Layers3 },
  { id: "owners", label: "Owners", icon: UserRound },
  { id: "notes", label: "QA notes", icon: MessageSquareText },
...
export function IssueEditorDialog({
  ...
  const developerMembers = workspace.members.filter((member) => memberHasRole(member, "developer"));
  const testerMembers = workspace.members.filter((member) => memberHasRole(member, "tester"));
  ...
  const reporterFallbackLabel = currentMember
    ...
    : "Signed-in reporter";
  const developmentAssignmentValue = getRoleAssignmentValue(form, workspace.members, "developer");
  const testingAssignmentValue = getRoleAssignmentValue(form, workspace.members, "tester");
  const canReopen = Boolean(editingIssue && canReopenIssue(editingIssue));
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92svh] overflow-hidden p-0 sm:max-w-6xl">
        <form className="flex max-h-[92svh] min-h-0 flex-col" onSubmit={onSubmit}>
            ...
            <div className="flex min-w-0 flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
              <div className="min-w-0">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  {form.reopen ? <Badge variant="destructive">Reopen pending</Badge> : null}
                </div>
                <DialogTitle className="text-xl tracking-tight">
                  {editingIssue ? "Edit issue" : "Create issue"}
                </DialogTitle>
                <DialogDescription>
                  Track the lifecycle as status, then use development and deployment to show the exact
                  handoff point.
                </DialogDescription>
              </div>
              {canReopen ? (
                <Button
                  type="button"
                  variant={form.reopen ? "destructive" : "outline"}
                  size="sm"
                  onClick={markForReopen}
                  disabled={pending}
          ...
          <div className="grid min-h-0 flex-1 lg:grid-cols-[13rem_minmax(0,1fr)]">
            <aside className="hidden border-r border-border/70 bg-muted/20 p-4 lg:block">
              <div className="text-xs font-medium uppercase text-muted-foreground">Issue workflow</div>
              <nav className="mt-4 space-y-1">
                {workflowSections.map((section, index) => (
                  <a
                    key={section.id}
                    href={`#issue-editor-${section.id}`}
            ...
            <div className="tracker-thin-scrollbar min-h-0 overflow-y-auto px-5 py-5">
                ...
                <main className="min-w-0 space-y-5">
                  <section id="issue-editor-issue" className="space-y-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-3">
                      ...
                      <Field label="Status">
                        <Select value={form.status} onValueChange={(value) => updateStatus(value as IssueStatus)}>
                          <SelectTrigger className="w-full">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                  ...
                  <section id="issue-editor-notes" className="space-y-4 border-t border-border/70 pt-5">
                ...
                <aside className="grid content-start gap-5 md:grid-cols-2 xl:grid-cols-1">
                  <section id="issue-editor-planning" className="space-y-4">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                      ...
                      <Field label="Sprint">
                        <Select value={form.sprintId} onValueChange={(value) => updateForm({ sprintId: value })}>
                          <SelectTrigger className="w-full">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                      ...
                      <Field label="Release">
                        <Select value={form.releaseId} onValueChange={(value) => updateForm({ releaseId: value })}>
                          <SelectTrigger className="w-full">
                            <SelectValue />
                          </SelectTrigger>
                          <SelectContent>
                    ...
                    <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-1">
                      <Field label="Development owner">
                        <RoleAssignmentSelect
                          ...
                          onChange={(value) =>
                            updateForm(parseRoleAssignmentValue(value, "developer"))
                          }
                        />
                      </Field>
                      <Field label="Testing owner">
                        <RoleAssignmentSelect
                          ...
                          currentUserId={currentUserId}
                          onChange={(value) => updateForm(parseRoleAssignmentValue(value, "tester"))}
                        />
                      </Field>
                      <Field label="Reporter">
                        ...
                        >
                          ...
                          <SelectContent>
                            <SelectItem value={NONE_VALUE}>{reporterFallbackLabel}</SelectItem>
  ...
  return (
    <section className="rounded-lg border border-border/70 bg-muted/20 p-3">
      <div className="grid gap-2 md:grid-cols-4">
        <WorkflowStep icon={AlertCircle} label="To Do" active={form.status === "todo"} />
        <WorkflowStep icon={ClipboardList} label="In Progress" active={form.status === "in_progress"} />
        <WorkflowStep icon={TestTube2} label="In Review" active={form.status === "review"} />
        <WorkflowStep icon={CheckCircle2} label="Fixed" active={form.status === "fixed"} />
      </div>
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        <Field label="Status">
          <Select value={form.status} onValueChange={(value) => onStatusChange(value as IssueStatus)}>
        ...
        <Field label="Development">
          <Select
            ...
            onValueChange={(value) => onDevelopmentChange(value as DevelopmentStatus)}
          >
            <SelectTrigger className="w-full bg-background">
              <SelectValue />
            </SelectTrigger>
        ...
        <Field label="Deployment">
          <Select
            ...
            onValueChange={(value) => onDeploymentChange(value as DeploymentStatus)}
          >
            <SelectTrigger className="w-full bg-background">
              <SelectValue />
            </SelectTrigger>
...
function inferAssignmentGroup(member: TeamMember | undefined, role: AssignmentRole): IssueAssignmentGroup | null {
...
function getRoleAssignmentValue(form: IssueFormState, members: TeamMember[], role: AssignmentRole) {
  const assigneeId = role === "developer" ? form.assigneeId : form.testerAssigneeId;
  const assigneeGroup = role === "developer" ? form.assigneeGroup : form.testerAssigneeGroup;
  ...
  if (assigneeId !== NONE_VALUE) {
    ...
    const group = assigneeGroup !== NONE_VALUE ? assigneeGroup : inferAssignmentGroup(member, role);
...
function parseRoleAssignmentValue(value: string, role: AssignmentRole): Partial<IssueFormState> {
...
function canReopenIssue(issue: IssueListItem) {
  return issue.status === "review" || issue.status === "fixed";
}
