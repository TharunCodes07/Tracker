        ...
        <Button
          ...
          className={cn(
            ...
            mediaItem.mediaType === "image"
              ? "border-sky-500/25 bg-sky-500/10 text-sky-700 hover:text-sky-700 dark:text-sky-300 dark:hover:text-sky-300"
              : "border-violet-500/25 bg-violet-500/10 text-violet-700 hover:text-violet-700 dark:text-violet-300 dark:hover:text-violet-300",
            compact && "w-7 px-0"
          )}
...
export function IssueMediaAttachmentList({
  issueId,
  media,
  canRemove = false,
  disabled,
      ...
      <div className="divide-y divide-border/60 border-y border-border/70">
        {media.map((mediaItem) => (
          ...
          >
            ...
            >
              {getMediaIcon(mediaItem.mediaType, "h-4 w-4")}
            </div>
            <button
              type="button"
              className="min-w-0 flex-1 text-left"
              onClick={() => openExistingMedia(issueId, mediaItem)}
            ...
            >
              ...
              <span className="block text-xs text-muted-foreground">
                {MEDIA_LABEL[mediaItem.mediaType]} - {formatBytes(mediaItem.sizeBytes)}
              </span>
            </button>
            <Button
              type="button"
              ...
              onClick={() => openExistingMedia(issueId, mediaItem)}
              aria-label={`Open ${mediaItem.originalName}`}
            >
            ...
            {canRemove ? (
              <Button
                ...
                variant="ghost"
                size="icon-sm"
                className="shrink-0 text-destructive opacity-0 transition-opacity hover:text-destructive group-hover:opacity-100 group-focus-within:opacity-100"
                disabled={disabled}
                onClick={() => onRemove?.(mediaItem.id)}
                aria-label={`Remove ${mediaItem.originalName}`}
              >
